...
author: Andrew Leach
date: <p>`r format(Sys.time(), '%d %B, %Y')`</p>
output:
  html_document:
      code_folding: hide
      includes:
...
#if you want to create a png in normal code, use this
#ggsave(oil_sands,"oil_sands_prod.png",width=12,height=6)
```
...
```{r, echo = T,eval=T}
  annual%>%filter(product%in%c("Conventional Light Oil Production","Conventional Heavy Oil Production","Total Oil Sands Production","Total Production")) %>%
  mutate(production=m3_bbl(production)/10^6/365)%>%select(-yoy_growth)%>%pivot_wider(names_from=product,values_from = production)%>%
  rename(Year=year)%>%
